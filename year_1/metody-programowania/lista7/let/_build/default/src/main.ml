...
let rec subst (x : ident) (s : expr) (e : expr) : expr =
  ...
  | Sum (y, e1, e2, e3) -> if x = y
                        then Sum (y, subst x s e1, subst x s e2, e3)
                        else Sum (y, subst x s e1, subst x s e2, subst x s e3) 
  | _ -> e
  ...
  | Sum (x, n, m, k) -> 
      ...
      | (VInt (a), VInt (b)) ->
        let rec helper curr acc =
          if curr > b then acc
          else let r = eval (subst x (Int curr) k) in
            helper (curr + 1) (eval_op Add r acc)
        in helper a (VInt 0)
      | _, _ -> failwith "type error")
      
let closed (e : expr) : bool =
  let rec helper e bounded = 
    match e with
    | Let (x, e1, e2) -> helper e1 bounded && helper e2 (x :: bounded)
    | Var x -> List.mem x bounded
    | Int _ | Bool _ -> true
    | Binop (_, e1, e2) -> helper e1 bounded && helper e2 bounded
    | If (p, t, e) -> helper p bounded && helper t bounded && helper e bounded
    | Sum (_, _, _, _) -> failwith "not implemented"
  in
  helper e []
